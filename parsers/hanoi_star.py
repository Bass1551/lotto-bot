# -*- coding: utf-8 -*-
"""Parser for Hanoi Star (minhngocstar.com)."""
...
class HanoiStarParser(BaseParser):
    name = "ฮานอย Star"
    ...
    def _extract(self, soup: BeautifulSoup) -> dict[str, str]:
        ...
        candidates = []
        for sel in [".result", ".result-number", ".ketqua", "h1", "h2", ".number"]:
...
def create_parser(url: Optional[str] = None) -> HanoiStarParser:
    return HanoiStarParser(url=url)
