# -*- coding: utf-8 -*-
"""Parser for Laos HD (laoshd.com).

Important: For Laos lotteries we must extract the displayed
3-digit (บน) and 2-digit (ล่าง) fields directly.
Do NOT derive them from a 5-digit number.
"""
...
class LaoHdParser(BaseParser):
    ...
    def _extract(self, soup: BeautifulSoup) -> dict[str, str]:
        ...
        if not (top3 and bottom2):
            ...
            for el in soup.select(".result, .number, td, .prize, h1, h2, h3"):
                t = el.get_text(strip=True)
                if t in top_block:
                    d = extract_digits(t)
...
def create_parser(url: Optional[str] = None) -> LaoHdParser:
    return LaoHdParser(url=url)
