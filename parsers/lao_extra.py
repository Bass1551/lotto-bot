# -*- coding: utf-8 -*-
"""Parser for Laos Extra (laoextra.com)."""
...
class LaoExtraParser(BaseParser):
    name = "ลาว Extra"
    ...
    def _extract(self, soup: BeautifulSoup) -> dict[str, str]:
        ...
        if not (top3 and bottom2):
            ...
            for el in soup.select(".result-number, .result, .number, td, h1, h2"):
                t = el.get_text(strip=True)
                if t in top_block:
                    d = extract_digits(t)
...
def create_parser(url: Optional[str] = None) -> LaoExtraParser:
    return LaoExtraParser(url=url)
